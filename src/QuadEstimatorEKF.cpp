...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F acceleration_inertial = attitude.Rotate_BtoI(accel);

  predictedState(3) += acceleration_inertial.x * dt;
  predictedState(4) += acceleration_inertial.y * dt;
  predictedState(5) += (acceleration_inertial.z - CONST_GRAVITY) * dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  /*Rbg(0, 0) = cos(pitch) * cos(yaw);
  Rbg(0, 1) = sin(roll) * sin(pitch) * cos(yaw) - cos(roll) * sin(yaw);
  Rbg(0, 2) = cos(roll) * sin(pitch) * cos(yaw) + sin(roll) * sin(yaw);
  Rbg(1, 0) = cos(pitch) * sin(yaw);
  Rbg(1, 1) = sin(roll) * sin(pitch) * sin(yaw) + cos(roll) * cos(yaw);
  Rbg(1, 2) = cos(roll) * sin(pitch) * sin(yaw) - sin(roll) * cos(yaw);
  Rbg(2, 0) = -sin(pitch);
  Rbg(2, 1) = cos(pitch) * sin(roll);
  Rbg(2, 2) = cos(pitch) * cos(roll);*/
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  Quaternion<float> attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, newState(6));

  V3F acceleration_inertial = attitude.Rotate_BtoI(accel);

  double RbgPrime0 = RbgPrime(0, 0) * acceleration_inertial[0] + RbgPrime(0, 1) * acceleration_inertial[1] + RbgPrime(0, 2) * acceleration_inertial[2];
  double RbgPrime1 = RbgPrime(1, 0) * acceleration_inertial[0] + RbgPrime(1, 1) * acceleration_inertial[1] + RbgPrime(1, 2) * acceleration_inertial[2];
  double RbgPrime2 = RbgPrime(2, 0) * acceleration_inertial[0] + RbgPrime(2, 1) * acceleration_inertial[1] + RbgPrime(2, 2) * acceleration_inertial[2];

  gPrime(3, 6) = RbgPrime0 * dt;
  gPrime(4, 6) = RbgPrime1 * dt;
  gPrime(5, 6) = RbgPrime2 * dt;

  MatrixXf ekfCov_last = ekfCov;

  MatrixXf gPrimeT = gPrime.transpose();

  ekfCov = gPrime * ekfCov_last * gPrimeT + Q;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float yawDifference = z(0) - zFromX(0);
